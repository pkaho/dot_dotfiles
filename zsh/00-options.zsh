...
# 通用配置
# ------------------------------
WORDCHARS=${WORDCHARS//[\/]} # 移除 / 分隔符, 命令行跳转时会在 / 停留
bindkey -e                   # 编辑模式(-e/-v), 等效 setopt EMACS/VI
...
HISTSIZE=12345          # 内存中保留历史条数
SAVEHIST=12345          # 写入文件最大历史条数
...
setopt CORRECT                # 自动纠错命令拼写错误, 使用 SPROMPT 自定义提示样式
...
# 后台任务
# ------------------------------
setopt NO_BG_NICE     # 后台进程不自动降低运行优先级
setopt NO_CHECK_JOBS  # 退出终端不检查后台/暂停任务, 建议搭配 NO_HUP
setopt NO_HUP         # 退出终端不杀死后台进程(不发送SIGHUP)
